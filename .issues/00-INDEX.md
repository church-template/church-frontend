...
**1차 배치(T1~T16) 범위:** 공개 + 회원 프론트만. 1차 콘텐츠는 백엔드 API/DB 시드로 투입 전제.
**2차 배치 — 어드민 트랙:** `/api/admin/**` 쓰기 전면 연동. 이슈는 [`admin/`](admin/00-epic.md) 폴더(에픽 `00` + 하위 `01`~`07`).
...
백엔드 `/api/admin/**` 쓰기 45개·14개 도메인 연동. **진입 모델**: ① 공개 페이지 인라인 RBAC 액션(권한 보유 시 등록·수정·삭제 노출) · ② 마이페이지 관리 허브(`/mypage`에서 한눈에) · ③ 공개 화면 없는 운영 도메인은 `/mypage/manage/*` 전용 화면. 권한 게이팅은 `useMe().permissions` 단일 기준. 상세는 [`admin/00-epic.md`](admin/00-epic.md).

| # | 이슈 | 라벨 | 선행 | 핵심 |
|---|---|---|---|---|
| 00 | 어드민 트랙 에픽 | `admin`·`epic` | T1~T16 | 진입 모델·하위 묶음·의존 그래프·진행 순서 |
| 01 | 어드민 기반 인프라 | `admin`·`infra` | T5·T6·T15 | RBAC 게이트·쓰기 헬퍼+무효화·DataTable·폼 프리미티브(MD에디터·태그·날짜·업로더)·관리 허브 셸 |
| 02 | 콘텐츠 관리(설교·공지) | `admin`·`content` | 01·T10·T11 | 마크다운 본문+태그+낙관락, 전용 편집 라우트 + 인라인 액션 |
| 03 | 일정 관리 | `admin`·`content` | 01·T12 | 캘린더/상세 인라인 CRUD, 종료>시작 검증·낙관락 |
| 04 | 부서 관리 | `admin`·`content` | 01·T9 | 부서 계층 CRUD, 전용 화면(공개 인트로=상수구동)·자식 삭제 차단 |
| 05 | 미디어 기반(갤러리·주보·미디어) | `admin`·`media` | 01·T13·T16 | 업로드 FormData·차단형 삭제(MEDIA_IN_USE)·갤러리/주보 인라인 |
| 06 | 분류·운영(태그·직분) | `admin`·`ops` | 01 | name CRUD 전용 화면, 삭제 비차단·중복 409 |
| 07 | 회원·권한 거버넌스 | `admin`·`governance`·`security` | 01 | 회원·역할·권한·약관 리셋, priority 위계 가드 |
...
01(기반) ─┬─ 02 (설교·공지)
          ├─ 03 (일정)
          ├─ 04 (부서)
          ├─ 05 (갤러리·주보·미디어)  ← 01 MediaUploader
          ├─ 06 (태그·직분)
          └─ 07 (회원·권한)
```

권장 진행 순서: **01 → 02 → 03 → 05 → 04 → 06 → 07**.
