...
describe("formatEventTime", () => {
  ...
  it("allDay 여러 날은 종료일만 '~ M. D.'", () => {
    expect(
      formatEventTime("2026-06-14T00:00:00", "2026-06-15T00:00:00", true),
    ).toBe("~ 6. 15.");
  });
  ...
  it("같은 날 범위는 '시각 ~ 시각'", () => {
    expect(
      formatEventTime("2026-06-14T10:00:00", "2026-06-14T12:00:00", false),
    ).toBe("10:00 ~ 12:00");
  });

  it("다른 날 범위는 종료일을 한정해 표기한다", () => {
    expect(
      formatEventTime("2026-06-14T10:00:00", "2026-06-15T12:00:00", false),
    ).toBe("10:00 ~ 6. 15. 12:00");
  });
});
```
...
describe("SermonCard", () => {
  it("썸네일 없이 텍스트형으로 렌더한다(보조줄·태그·링크)", () => {
    ...
    expect(
      screen.getByText("요한복음 시리즈 · 요한복음 20:19-23"),
    ).toBeDefined();
    ...
    expect(
      screen.getByText("부활의 증인").closest("a")?.getAttribute("href"),
    ).toBe("/sermons/1");
  });

  it("series만 있으면 보조줄에 구분점 없이 표기한다", () => {
    render(
      <SermonCard title="t" preacher="p" date="d" series="요한복음 시리즈" />,
    );
  ...
  const inner = (
    <>
      ...
      <div className="p-xl">
        ...
        <h3 className={cn(typo.titleMd, "text-ink", subtitle && "mt-xxs")}>
          {title}
        </h3>
...
describe("EventCard", () => {
  it("시간·장소가 있으면 메인 일정 카드로 렌더한다(summary 없음)", () => {
    ...
    expect(
      screen.getByText("성가대 연습").closest("a")?.getAttribute("href"),
    ).toBe("/events/3");
  });
...
describe("NoticeRow", () => {
  it("isPinned면 '고정' 배지를 표시한다", () => {
    ...
    expect(
      screen.getByText("수련회 안내").closest("a")?.getAttribute("href"),
    ).toBe("/notices/2");
  });

  it("기본은 배지 없이 제목·날짜만", () => {
    render(
      <NoticeRow title="일반 공지" date="2026. 6. 3." href="/notices/3" />,
    );
...
export function SermonSection({ sermons }: { sermons: SermonCardResponse[] }) {
  return (
    <section className="bg-surface-soft py-section">
      <Container>
        <h2 className={cn(typo.displayLg, "text-ink")}>
          {MAIN_SECTIONS.sermons.title}
        </h2>
        {sermons.length === 0 ? (
...
describe("NoticeSection", () => {
  it("행 매핑 — 제목·날짜 포맷·고정 배지·상세 링크", () => {
    ...
    expect(screen.getByText("고정")).toBeDefined();
    expect(
      screen.getByText("전교인 수련회 안내").closest("a")?.getAttribute("href"),
    ).toBe("/notices/2");
  });
...
- Create: `public/hero-poster.jpeg` (생성 산출물)
...
grad.resize((W, H)).save("public/hero-poster.jpeg", quality=85)
print("saved public/hero-poster.jpeg")
PY
```

- [ ] **Step 2: 산출물 확인**

Run: `ls -la public/hero-poster.jpeg && file public/hero-poster.jpeg`
Expected: JPEG image data, 1920×1080, 수십 KB

- [ ] **Step 3: Commit**

```bash
git add public/hero-poster.jpeg
