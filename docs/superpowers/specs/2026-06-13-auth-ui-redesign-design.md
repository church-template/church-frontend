...
- **좌측 패널** (데스크톱 `md:flex` 50% 폭(`w-1/2`), `min-h-dvh`): `HERO.poster`(hero-poster.jpeg) 배경 cover + `cover-dark` 계열 반투명 덮개. 좌상단 `CHURCH_NAME` 로고(홈 `/` Link, on-dark), 좌하단 `HERO_CAPTION` 슬로건(`typo.titleLg`, on-dark) — **전부 기존 상수 주입, 하드코딩 0**
