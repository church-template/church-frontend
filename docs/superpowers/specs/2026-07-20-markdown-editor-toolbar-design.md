...
- 2026-07-20 실사용 피드백 반영:
  - **Dialog 삽입(링크·유튜브·이미지·표)은 execCommand를 시도하지 않고 상태 교체로 직행한다** — 삽입 시점에
    Dialog가 아직 열려 있어 포커스 트랩이 `focus()`를 되가져가고, execCommand("insertText")가 다이얼로그
    입력에 꽂힌 채 true를 반환해 폴백까지 막혀 본문이 갱신되지 않는 실브라우저 버그. 직접 툴바 버튼은
    기존대로 execCommand 우선(사용자 실측으로 정상 확인).
  - **미리보기 표 스타일 추가**: `.prose-church` table/th/td(hairline 보더, surface-soft 헤더, body-md) —
    스타일 부재로 렌더된 표가 테두리 없는 맨 텍스트처럼 보였다.
  - **표 버튼을 크기 입력 Dialog로 전환**: 고정 2×2 템플릿 대신 열(1~8)·행(1~20)을 숫자로 입력(기본 2×2),
    자리표시 헤더(제목1…)+구분행+빈 본문 행을 `buildTable`(순수 함수)로 생성. 세 번째 Dialog 등장을 계기로
    `InsertDialogShell` 공용 뼈대를 추출했다.
