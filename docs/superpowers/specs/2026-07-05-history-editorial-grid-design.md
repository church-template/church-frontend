...
| H4 | **기각: 스크롤 연출 추가** *(부분 개정 2026-07-05)* | 기존 `Reveal`(fade+slide-up) 유지가 원칙. 단 사용자 요청으로 **사진 셀 방향성 와이프 1건만 채택**(§5) — 그 외 연출(카운트업 등)은 계속 기각 |
...
- 사진 셀 방향성 와이프(H4 부분 개정): 챕터 Reveal 등장의 공개 신호(`data-revealed`
  속성 — 모듈 해시 클래스는 외부 CSS가 선택 불가)와 동기해 `clip-path: inset` 전환
  (0.7s ease·0.15s 지연, Reveal 0.6s에 겹침). 사진이 우측(표준)=좌→우, 좌측(미러·
  데스크톱만)=우→좌, 모바일 스택은 좌우 의미가 없어 일괄 좌→우. reduced-motion은
  클립 없이 정적
