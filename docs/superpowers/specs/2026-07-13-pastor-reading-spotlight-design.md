...
- **모바일·짧은 뷰포트 = 비고정**: 콘텐츠(초상 + 본문)가 화면보다 커서 고정하면 잘린 채 멈춘다.
  대신 각 문장이 화면 중앙대(`cover 35%~70%`)를 지나는 동안 켜진다.
...
```css
.stage    { height: calc(100vh + var(--sentences) * 20vh); view-timeline-name: --pastor-read; }
.sentence { animation-timeline: --pastor-read; }
```

고정된 채 스크롤되는 길이 = `stage 높이 - 100vh` = `문장수 × 20vh` — 본문이 길어지면 고정도 그만큼 길어진다.
...
| 좁은/낮은 뷰포트 | 고정 없음, 문장이 화면 중앙대를 지날 때 켜짐 |
...
3. Playwright — 데스크톱(1440×900): 고정 구간 8%/42%/92% 에서 켜진 문장이 1→3→6 으로 하나씩 이동, 섹션은
   멈춰 있음. 모바일(390×844): 고정되지 않고 문장이 중앙대에서 차례로 켜짐.
4. `pnpm test` · `npx tsc --noEmit` · `pnpm lint`.

## 노브 (조정 가능한 값)

- **문장당 읽는 시간**: `.stage` 의 `20vh` (키우면 한 문장을 더 오래 붙든다).
