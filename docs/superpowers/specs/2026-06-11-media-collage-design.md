...
| C5 | 스크럽 방식 (데스크톱) | 참조와 동일하게 스크롤 진행도에 비례(가역). 1회 재생 entrance는 기각 — 요청된 질감과 다름 |
| C6 | **모바일 = 세로 스택 카드** (2026-06-12 개정) | 모바일에선 sticky 스크럽을 버리고 일반 흐름: 중앙 카드 아래 타일 4장이 세로로 쌓이며, 화면 진입 시 좌/우 번갈아 슬라이드 인(1회, IO+CSS transition). 터치 스크롤에 자연스럽고 4장 전부 노출. 기존 "2타일 축소 콜라주"는 폐기 |
| C7 | 데스크톱 재배치 (2026-06-12 개정) | 중앙을 세로형(32vw×72vh)으로 좁히고 타일을 안쪽으로 끌어와 상하 어긋남(지그재그) 리듬 — 와이드 화면에서 중앙이 정사각형으로 비대해지고 타일이 가장자리에 붙어 빈 공간이 크던 문제 해소 |
...
- **중앙 카드 축소** — p 0→0.55, `easeOut(t) = 1 - (1-t)³` (처음 빠르게 빠지고 끝에서 안착):
  - `clip-path: inset(V% H% round Rpx)` 스크럽 — V 0→14, H 0→34, R 0→24 (C7 개정)
  - R(라운드)은 하드코딩하지 않고 **`--radius-xl` 토큰을 `getComputedStyle`로 1회 읽어** 합성
  - 결과: 풀스크린 → 중앙 **세로형** 카드(32vw × 72vh). 카드 밖은 `bg-canvas`
- **타일 슬라이드 인** — transform+opacity만, 타일별 구간을 어긋나게(패럴랙스 질감).
  타일은 안쪽으로 붙이고 상하를 어긋나게(T1↑T3↓·T2↓T4↑) 배치한다(C7 개정):

| 슬롯 | 위치·크기 (데스크톱) | 진입 방향 | 구간 |
|---|---|---|---|
| T1 좌상 | left 14% · top 10% · w 15vw · 3:4 | 위에서 ↓ (translateY -40vh→0) | p 0.15~0.70 |
| T2 좌하 | left 7% · bottom 12% · w 19vw · 4:3 | 왼쪽에서 → (translateX -50vw→0) | p 0.25~0.80 |
| T3 우상 | right 7% · top 16% · w 18vw · 16:10 | 오른쪽에서 ← (translateX 50vw→0) | p 0.20~0.75 |
| T4 우하 | right 13% · bottom 8% · w 14vw · 3:4 | 아래에서 ↑ (translateY 40vh→0) | p 0.30~0.85 |
...
- **모바일(<640px) = 세로 스택 카드(C6 개정)**: sticky·스크럽 없음, 일반 흐름.
  - 중앙 카드: 컨테이너 패딩 안 풀폭, `rounded-xl`(border-radius — 모바일은 clip-path 미사용)
  - 타일 4장: 세로 스택(w 76vw, 간격은 spacing 토큰), T1·T3 좌측 정렬·왼쪽에서 진입(translateX
    -40vw→0), T2·T4 우측 정렬·오른쪽에서 진입(+40vw→0) — 화면 진입 시 1회(IO threshold 0.25,
    CSS transition 0.6s ease), opacity 0→1 동반
  - 가로 오버플로 방지: 섹션에 `overflow-x: clip`
  - 브레이크포인트 전환(회전 등): `matchMedia("(max-width: 639px)")` 구독으로 메커니즘 재선택,
    데스크톱 스크럽이 남긴 인라인 스타일은 effect cleanup에서 제거
- **reduced-motion**: JS(스크럽·IO 모두) 미등록 + CSS 폴백 — 데스크톱은 핀 없이 **완성 상태
  콜라주를 정적 표시**, 모바일은 스택 카드를 transition 없이 즉시 표시.
...
| `MediaCollage` | 중앙 미디어 + 타일 4개 렌더 / (데스크톱) scroll·resize 리스너 등록·해제 / (모바일) IO 등록 + 교차 시 표시 클래스 부여 / reduced-motion 시 둘 다 미등록 / video 타일 onError→poster 폴백 / 장식 alt 처리 |
