...
| D3  | 히어로 에셋 = **플레이스홀더 생성**                           | `public/hero-poster.jpeg`(다크 네이비 그라데이션 1920×1080) 생성. `hero.mp4`는 ffmpeg 가용 시에만 생성 — 없으면 `onError→poster` 폴백이 기본 경로로 동작해 검수 항목이 자연 검증됨. 실제 에셋은 추후 파일 교체만                                                                                                                                                                                                  |
...
└─ public/hero-poster.jpeg             # 개발용 플레이스홀더 (hero.mp4는 ffmpeg 가용 시)
